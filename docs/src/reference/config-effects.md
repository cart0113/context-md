...
## Reactive posture {#reactive}

`remind-on-demand-read` and `remind-on-demand-update` set to `true`. The dispatcher appends reminders telling the agent to read or write context-db only when the user explicitly invokes a `/context-db` command.
...
decisions — wait for the user to explicitly run `/context-db update` or
`/context-db maintain`. If you think something belongs in context-db, mention it
...
update, or maintain). If you think context-db has relevant material for the
current task, mention it to the user and let them invoke the command.
